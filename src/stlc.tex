...
\begin{definition}[$\beta$-expansion closed]
  ...
  \leadsto t$ imply $t' \in S$. \index{$\beta$-expansion closed}
\end{definition}
...
\begin{definition}
  Let $\normterms$ be $\{ t\ |\ t \downarrow \}$.\index{$\mathcal{N}$}
\end{definition}

Suppose there is a base type $b\in B$, and define $I(b)$ to be $\normterms$.  Then we have:
...
\item $\lam{x}{\lam{y}{x}} \in \interp{b \to b}$.  To prove this, it suffices by the semantics
  ...
  we are assuming $t' \downarrow$, and need to show $(\lam{x}{\lam{y}{x}})\ t'\downarrow$.
  For the latter:
  \[
  \underline{(\lam{x}{\lam{y}{x}})\ t'} \leadsto \lam{y}{t'}
  \]
  \noindent and the latter is normalizing since $t'$ is.

\item Also, $\lam{x}{\lam{y}{x}} \in \interp{b}$, since $\lam{x}{\lam{y}{x}}$ is in normal
  form (and hence normalizing).
...
\begin{itemize}
\item $\lam{x}{x\ x} \in \interp{b}$, since $\lam{x}{x\ x}$ is in normal form.
...
Now let us change the interpretation $I(b)$ to be $\{ t \ |\ t \betaa^* \lam{x}{x}\}$.  Then we
