...
Terms are then certain labeled binary trees\index{term}.  We will use the
...
\]
  \caption{Recursive definition of capture-avoiding substitution as a
    partial function\index{substitution}\index{$[t'/x]t$}.  If a recursive call is undefined then the outer
...
we say that $t$ $\beta$-reduces to $t'$.  \index{$\beta$-reduction}\index{$\betar$}
\end{definition}
...
\begin{definition}[nested redex]
  ...
  Then we say that $R_1$ is a nested redex (nested within $R_2$).\index{$\beta$-redex!nested}
  \end{definition}
...
\label{def:alpha}
  The relation $=_\alpha$, called $\alpha$-equivalence, is $\Tau[\alpha]^*$.\index{$\alpha$-equivalence}\index{$=_\alpha$}
  \end{definition}
...
\begin{definition}[relational composition]
  ...
  $(x,y) \in R$ and $(y,z)\in S$.\index{composition!relational}
\end{definition}
...
$\beta$-reduction with renaming.\index{$\beta$-reduction!with renaming}\index{$\betaa$}
\end{definition}

A single $\beta$-reduction step with renaming allows one to perform some (possibly zero)
renamings, and then take a $\betar$-step.
We are generally interested in taking multiple steps of $\betaa$, using $\betaa^*$, which
we will call multi-step $\beta$-reduction with renaming.\index{$\beta$-reduction!multi-step with renaming}
...
\label{def:betars}
  ...
  \]
  We say this is a $\betaa$-reduction sequence for $t_1\betaa^* t_k$.\index{reduction sequence}
\end{definition}
...
\begin{definition}[maximal relational sequence]
  ...
  there is no $t'$ with $t_n\ R \ t'$.\index{reduction sequence!maximal}
  \end{definition}
...
\label{def:normalizing}
  ...
  If $R$ is left off, we assume it is $\betaa$.\index{normalizing term}\index{$t\downarrow_R$}
\end{definition}
...
\label{def:nonnorm}
  ...
  As above, if $R$ is left off, we assume it is $\betaa$.\index{non-normalizing term}\index{$t\uparrow_R$}
\end{definition}
...
\label{def:betaeq}
  We define $\betaeq$ as the equivalence closure of $\betaa$; i.e., $=_{\betaa}$.\index{$\beta$-equivalence}{$\betaeq$}
  \end{definition}

\section{Combinators}

Haskell Curry developed an alternative approach to defining computable functions, without any bound variables.  The
syntax of these combinators is:
\[
\textit{combinators}\ c\  ::= \ S\ |\ K\ |\ x\ |\ c\ c'
\]
\noindent where free variables $x$ are included to make it easier to express the translation (below) from $\lambda$-calculus
to combinators.

The semantics of combinators is given in Figure~\ref{fig:combsem}, where we use the same symbol $\betaa$ as we did for
$\beta$-reduction with renaming.  Here, though, as there are no bound variables, there is no renaming.

\begin{figure}
  \[
  \begin{array}{lllllll}
    \infer{S\ t_1\ t_2\ t_3\ \betar\ t_1\ t_3\ (t_2\ t_3)}{\ } & \ &
\infer{K\ t_1\ t_2\ \betar\ t_1}{\ } & \ &    
\infer{t_1\ t_2\ \betar\ t_1'\ t_2}{t_1\ \betar\ t_1'} &\ &
\infer{t_1\ t_2\ \betar\ t_1\ t_2'}{t_2\ \betar\ t_2'}
  \end{array}
  \]
  \caption{Inference rules defining the reduction relation for combinators. }
  \label{fig:combsem}
\end{figure}
