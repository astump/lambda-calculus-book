...
\begin{definition}[Subterm at a position]
  ...
  \begin{eqnarray*}
    t|_\epsilon & = & t \\
    (t_1\ t_2)|_{i\pi} & = & t_i|_\pi \\
    (\lam{x}{t})|_{1} & = & x \\
    (\lam{x}{t})|_{2\pi} & = & t|_\pi
  \end{eqnarray*}
\end{definition}
...
If $t\ \leadsto_R^*\ t'$, then $\lam{x}{t}\ \leadsto_R^*\ \lam{x}{t'}$.
\end{lemma}
\begin{proof}
...
\begin{proof}
  ...
  \case{ } 
  \[
  \infer{t\ \leadsto_{\leadsto_R^*}\ t'}{t \leadsto_R^* t'}
  \]
  \noindent The desired conclusion for this inclusion inference is its premise: $t \leadsto_R^* t'$.

  \case{ }
  \[
  \infer{\lam{x}{t}\ \leadsto_{\leadsto_R^*}\ \lam{x}{t'}}{t\ \leadsto_{\leadsto_R^*}\ t'}
  \]
  \noindent By the induction hypothesis, we have $t\ \leadsto_R^*\ t'$.  The result
